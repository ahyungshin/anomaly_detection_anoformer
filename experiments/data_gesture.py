...
def load_data_gesture(opt):

    #- data
    with open(str(os.path.join(opt.dataroot, "ann_gun_CentroidA_train.pkl")), 'rb') as f:
        N_samples = torch.FloatTensor(pickle.load(f)) 
        N_samples_y = N_samples[:,-1] # 8251
        N_samples = N_samples[:,:-1] # 8251,2

    with open(str(os.path.join(opt.dataroot, "ann_gun_CentroidA_test.pkl")), 'rb') as f:
        data = torch.FloatTensor(pickle.load(f)) 
        test_label = data[:,-1] # 3000
        test_data = data[:,:-1] # 3000,2


    N_samples = normalize(N_samples) 
    test_data = normalize(test_data) 

    N_samples = N_samples.unfold(0,100, 1)
    N_samples_y = N_samples_y.unfold(0,100, 1)
    test_data = test_data.unfold(0,100, 1)
    test_label = test_label.unfold(0,100, 1)

    N_samples = N_samples.reshape(N_samples.shape[0], 1, -1)
    N_samples_y = N_samples_y.reshape(N_samples_y.shape[0], 1, -1)
    ...
    N_samples_y = torch.any(N_samples_y.type(torch.bool), axis=-1).type(torch.float32)
    val_AN_y = torch.any(val_AN_y.type(torch.bool), axis=-1).type(torch.float32)
    test_AN_y = torch.any(test_AN_y.type(torch.bool), axis=-1).type(torch.float32)
