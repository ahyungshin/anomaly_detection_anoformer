...
def save_plot_pair_sample(samples1,samples2, idx, identifier, n_samples=6, num_epochs=None,impath=None ,ncol=2):
   ...
   if identifier=="ecg":
       for m in range(nrow):
           ...
           axarr[m, 0].set_ylim(0,400)
           axarr[m, 1].set_ylim(0,400)
...
def save_pair_fig(input,output,score,save_path):
    '''
    ...
    :return:
    '''
    save_ts_heatmap(input,output,score,save_path)
...
def save_ts_heatmap(input,output,score,save_path):
    ...
    ax[0].plot(x_points,sig_out,'k-',linewidth=2.5,label="output signal", color='red')
    ax[0].text(1.0, 370, 'anomaly score: %.3f'%score)
...
def auc_plot(hist, path = 'Train_hist.png', model_name = ''):
    x = range(len(hist['auc']))

    y = hist['auc']
    fig = plt.figure()

    ax1=fig.add_subplot(111)
    ax1.plot(x, y,'r',label="AUC")
    ax1.set_ylabel('AUC')

    path = os.path.join(path, model_name + '_auc.png')
